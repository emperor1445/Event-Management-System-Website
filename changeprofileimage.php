...
if(isset($_POST['change'])){
    ...
    $file_tmp = $_FILES['profimg']['tmp_name'];
        ...
        if($usertype=="student"){
            if(move_uploaded_file($file_tmp, "img/uploaded/studentprofile/".$file_name)){
                $sql = "UPDATE student SET SDimg = :img WHERE SDusername = :username";
                ...
                location.replace(\"changeprofileimage.php\");</script>";
            }
    
        }
        else{
            if(move_uploaded_file($file_tmp, "img/uploaded/staffprofile/".$file_name)){
                $sql = "UPDATE staff SET STimg = :img WHERE STUsername = :username";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(':img', $file_name);
                $stmt->bindParam(':username', $username); 
                $stmt->execute();
